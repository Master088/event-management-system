  ...
  <nav class="p-0 mt-0">
    <ul class="pagination justify-content-end m-0">
      ...
      <li v-for="page in pages" :key="page.name" class="page-item" :class="{ active: isPageActive(page.name) }">
        <a class="page-link" href="#" @click.prevent="onClickPage(page.name)" :disabled="page.isDisabled">
          {{ page.name }}
        </a>
      </li>
  ...
  export default {
    ...
    computed: {
      ...
      pages() {
        ...
        for (let i = this.startPage < 1 ? 1 : this.startPage; i <= Math.min(this.startPage + this.maxVisibleButtons - 1, this.totalPages); i++) {
          range.push({
            name: i,
...
<style>
  ...
  .active > .page-link {
    background-color: #2b99be !important;
    border-color: #2b99be !important;
